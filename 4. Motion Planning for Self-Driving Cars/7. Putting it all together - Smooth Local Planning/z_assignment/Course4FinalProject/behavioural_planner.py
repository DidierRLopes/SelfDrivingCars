...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            mid_index, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)
            if stop_sign_found:
                self._goal_index = mid_index
            else:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        for i in range(wp_index + 1, len(waypoints)):
            arc_length += math.sqrt((waypoints[i][0] - waypoints[i-1][0])**2
...
def get_closest_index(waypoints, ego_state):
    ...
    for i, wp in enumerate(waypoints):
        waypoint_len = np.linalg.norm(np.subtract(wp[0:2], ego_state[0:2]))
        if waypoint_len < closest_len:
            closest_index = i
            closest_len = waypoint_len
    # ------------------------------------------------------------------
